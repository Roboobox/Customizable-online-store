    ...
    ';
        ...
        $cartHtml .= '
                <div class="mt-3 ms-md-1 me-md-0 me-1 mb-2 mb-md-0">
                    <a onclick="return confirm(\'Are you sure you want to delete this item\')" href="cart_remove_item.php?' . ((isset($_SESSION['user_id'])) ? 'cart_id=' . $cartId . '&' : '') . 'product_id=' . $cartProduct->id . ((isset($_SESSION['user_token'])) ? '&token=' . $_SESSION['user_token'] : '') . '" class="link-dark">
...
if ($error || empty($products)) {
    $cartFooterHtml .= '<button disabled class="ms-4 btn btn-primary cart-continue fs-5 fw-bold">Continue <i class="fas fa-arrow-right"></i></button>';
}
else {
    $cartFooterHtml .= '<button onclick="location.href=\'checkout.php\'" class="ms-4 btn btn-primary cart-continue fs-5 fw-bold">Continue <i class="fas fa-arrow-right"></i></button>';
}

echo json_encode(array('cart' => $cartHtml, 'footer' => $cartFooterHtml));
